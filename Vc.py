...
# ───────── PAGE DETECTORS ─────────
async def dob_page(page):
    return bool(await page.query_selector('select[name="month"], text=/birthday|date of birth/i'))
...
async def password_step(update, ctx):
    ...
    await snap(page, update.effective_chat.id, ctx, "opened")
...
async def username_step(update, ctx):
    ...
    for _ in range(10):
        ...
        await update.message.reply_text("❌ Username failed. Send another.")
        return USERNAME

    await snap(page, chat_id, ctx, "username_ok")

    if await otp_page(page):
        await update.message.reply_text("📩 OTP received? Send **6-digit OTP**")
        return OTP
...
async def otp_step(update, ctx):
    ...
    if await success_page(page):
        ...
        await update.message.reply_text(
            "⚠️ OTP submitted, but no success detected.\n"
